...
| 前台应用切换 | +1 | `UsageStatsManager` 检测到 ACTIVITY_RESUMED 且包名变化 |
| 充电/拔电 | +2 | `ACTION_POWER_CONNECTED` / `ACTION_POWER_DISCONNECTED` |
| 数据流量开关变化 | +1 | `ConnectivityManager` 检测移动数据开关动作（非流量变化） |
...
| `INTERNET` | SMTP 发件 |
| `ACCESS_NETWORK_STATE` | 检测移动数据开关变化 |
...
| `WAKE_LOCK` | 闹钟触发时唤醒 CPU |
| `PACKAGE_USAGE_STATS` | 检测前台应用切换（系统设置手动授权） |
| `QUERY_ALL_PACKAGES` | `UsageStatsManager` 查询所有包名 |

> 不使用无障碍服务，不读取短信/通讯录/相册/位置。
